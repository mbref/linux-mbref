...
static const struct user_regset native_regsets[] = {
        [REGSET_GPR] = {
                .core_note_type = NT_PRSTATUS, .n = ((PT_MSR)/sizeof(unsigned long)),  // .n = 33 (0-31 gpr, 32 pc)
                /* all registers are 32-bits for the Microblaze */
                .size = sizeof(unsigned long), .align = sizeof(unsigned long),
...
long arch_ptrace(struct task_struct *child, long request, long addr, long data)
{
	...
	case PTRACE_POKEUSR:
		...
		break;
        /* Doesn't work TO_REMOVE.  These are standard ptrace defined routines, so not defining here.
	 * see kernel/ptrace.c for more details on the general routines. ~BH */
	#if 0
